...
def test_terminal_expr_linear_2d_2():
    ...
    expr_0 = Matrix([[x1 * v[0] + dx1(v[0])], [x2 * v[1] + dx2(v[1])]])
    l_term_expected = (DomainExpression(domain.interior, expr_0),)
    assert l_term == l_term_expected
    # ...
    ...
    expr_1 = Matrix([
       [(x1**2 * nn[0] + x2**2 * nn[1]) * v[0]],
       [                                     0],
       [                                     0],
       [(x1**2 * nn[0] + x2**2 * nn[1]) * v[1]]
    ])
    l_term_expected = (BoundaryExpression(B1, expr_1),)
    assert l_term == l_term_expected
    # ...

    # ...
    g = Tuple(x1**2, x2**2)
    l = LinearForm(v, int_1(v * dot(g, nn)) + int_0(x1 * x2 * v))
    l_term = TerminalExpr(l, domain)
    ...
    l_term_expected = (DomainExpression(domain.interior, expr_0),
                       BoundaryExpression(B1, expr_1))
    assert l_term == l_term_expected
    # ...

    # ...
    l1 = LinearForm(v1, int_0(x1 * x2 * v1))
    l = LinearForm(v, l1(v))
    l_term = TerminalExpr(l, domain)
    ...
    g = Tuple(x1, x2)
    l1 = LinearForm(v1, int_0(x1 * x2 * v1))
    l2 = LinearForm(v2, int_1(dot(grad(v2), g)))
    l  = LinearForm(v, l1(v) + l2(v))
    l_term = TerminalExpr(l, domain)
    ...
    expr_1 = Matrix([
        [mu * (x1**2 * nn[0] + x2**2 * nn[1]) * v[0]],
        [                                          0],
        [                                          0],
        [mu * (x1**2 * nn[0] + x2**2 * nn[1]) * v[1]]
    ])
    l_term_expected = (DomainExpression(domain.interior, expr_0),
                       BoundaryExpression(B1, expr_1))
    assert l_term == l_term_expected
