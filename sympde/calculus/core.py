...
class LogicalGrad(BasicOperator):
    """
    Represents a generic Grad operator, without knowledge of the dimension.
    ...
    def eval(cls, *_args):
        ...
        if isinstance(expr, Add):
            ...
            try:
                a,b = expr._args
                return Cross(a, LogicalCurl(b)) - Cross(LogicalCurl(a), b) + Convect(a,b) + Convect(b,a)
...
#==============================================================================
class LogicalCurl(BasicOperator):
    """
    Represents a generic Curl operator, without knowledge of the dimension.
    ...
    def eval(cls, *_args):
        ...
        if isinstance(expr, Add):
            ...
            if vectors:
                if len(vectors) == 2:
                    ...
                    if isinstance(a, (Tuple, VectorTestFunction, VectorField,
                                      LogicalGrad)):
                        f = b ; F = a
                        return f*LogicalCurl(F) + Cross(LogicalGrad(f), F)
            ...
            try:
                a,b = expr._args
                return a * LogicalDiv(b) - b*LogicalDiv(a) + Convect(b, a) - Convect(a, b)
            except:
                return cls(expr, evaluate=False)

        elif isinstance(expr, LogicalGrad):
            return S.Zero

        elif isinstance(expr, LogicalCurl):
            f = expr._args[0]
            return LogicalGrad(LogicalDiv(f)) - LogicalLaplace(f)
...
#==============================================================================
class LogicalRot(BasicOperator):
    """
    Represents a generic 2D rotational operator.
...
#==============================================================================
class LogicalDiv(BasicOperator):
    """
    Represents a generic Div operator, without knowledge of the dimension.
    ...
    def eval(cls, *_args):
        ...
        if isinstance(expr, Add):
            ...
            if vectors:
                if len(vectors) == 2:
                    ...
                    try:
                        if isinstance(a, (Tuple, VectorTestFunction, VectorField)):
                            f = b ; F = a
                            return f*LogicalDiv(F) + Dot(F, LogicalGrad(f))
            ...
            a,b = expr._args
            return Dot(b, LogicalCurl(a)) - Dot(a, LogicalCurl(b))
...
#==============================================================================
class LogicalLaplace(BasicOperator):
    """
    Represents a generic Laplace operator, without knowledge of the dimension.
        ...
        if isinstance(expr, Add):
            ...
            if vectors:
                if len(vectors) == 1:
                    ...
                    f,g = vectors
                    b = f*cls(g) + g*cls(f) + 2 * Dot(LogicalGrad(f), LogicalGrad(g))
...
logical_grad    = LogicalGrad
logical_curl    = LogicalCurl
logical_rot     = LogicalRot
logical_div     = LogicalDiv
logical_laplace = LogicalLaplace
